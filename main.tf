...
variable "github_repo_name" {
  description = "您的 GitHub 仓库名 (e.g., 'rag-app-demo')"
  type        = string
}
...
output "apprunner_service_arn" {
  ...
  description = "复制到 GitHub Secret: APP_RUNNER_ARN（如果服务由 GitHub Actions 创建，此值可能为空）"
}
output "apprunner_access_role_arn" {
  value = aws_iam_role.apprunner_service_role.arn
  description = "App Runner 访问角色 ARN（用于从 ECR 拉取镜像）"
}
output "apprunner_instance_role_arn" {
  value = aws_iam_role.apprunner_instance_role.arn
  description = "App Runner 实例角色 ARN（用于运行时访问 Secrets Manager）"
}
